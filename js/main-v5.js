...
document.addEventListener('DOMContentLoaded', () => {
  ...
  function initCompareSlider() {
    ...
    // Set overlay img width to match container (rAF to avoid layout thrashing)
  ...
  // Lazy videos (wpdp06-v5 perf optimization)
  initLazyVideos();
});
...
function initLazyVideos() {
  ...
  const ioVids = document.querySelectorAll('video[data-lazy="io"]');
  if (ioVids.length && 'IntersectionObserver' in window) {
    ...
    ioVids.forEach(v => obs.observe(v));
  }
}
